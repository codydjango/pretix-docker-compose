...
logger = logging.getLogger('pretix.plugins.etherpay')




def etherpay_test(request, *args, **kwargs):
...
def redirect_view(request, *args, **kwargs):
    ...
    r = render(request, 'pretixplugins/etherpay/redirect.html', {
        'url': url,
    })
    r._csp_ignore = True
    return r
...
def charge_webhook(event, event_json, charge_id, rso):
    raise Exception('charge webhook')
    # prov = StripeCC(event)
    # prov._init_api()
    # try:
...
def source_webhook(event, event_json, source_id, rso):
    raise Exception('source webhook')
    # prov = StripeCC(event)
    # prov._init_api()
    # try:
...
class ReturnView(EtherpayOrderView, View):
    def get(self, request, *args, **kwargs):
        raise Exception('ReturnView')
        # prov = self.pprov
        # prov._init_api()
    ...
    def _redirect_to_order(self):
        raise Exception('_redirect_to_order')
